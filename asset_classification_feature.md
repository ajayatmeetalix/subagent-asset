...
## Status: ✅ Shipped (commit 6a85b9a)

The `confirm_asset_classification` job modal is fully implemented. This document serves as reference for the mock data schema and display rules, plus context for the next feature build.
...
## Seeding the Next Feature: Determine Legal Administration Path

The classification response gives us enough to pre-populate the legal path determination:

| From classification response | Use in legal path modal |
|---|---|
| `classification.assets` (with buckets) | Show which assets go to probate vs. non-probate |
| `plan.actions` | Pre-populate required next steps (filings, forms, deadlines) |
| `plan.blocked_paths` | Show unavailable procedures upfront |
| Bucket breakdown (count of PROBATE assets) | Inform whether formal probate is required |

**Legal path options to determine:**
- Probate (Independent Administration)
- Probate (Supervised)
- Trust Administration
- Small Estate Affidavit (if gross estate under threshold)
